...
""")
  ...
  try:
    ...
    # Get all payment handlers from the grouped dictionary
    supported_handlers = []
    payment_handlers_dict = discovery_data.get("payment_handlers", {})
    for grouped_handlers in payment_handlers_dict.values():
      supported_handlers.extend(grouped_handlers)
    ...
    item1 = item_create_request.ItemCreateRequest(
    ...
    payment_req = payment_create_request.PaymentCreateRequest(
      instruments=[],
    ...
    if not checkout_data.get("fulfillment") or not checkout_data[
      ...
      trigger_req = checkout_update_request.CheckoutUpdateRequest(
        id=checkout_id,
        line_items=[line_item1_update, line_item2_update],
        currency=checkout_data["currency"],
        payment=checkout_data["payment"],
    ...
    if checkout_data.get("fulfillment") and checkout_data["fulfillment"].get(
      ...
      if method.get("destinations"):
        ...
        if method.get("groups") and method["groups"][0].get("options"):
          ...
          group_id = group["id"]
          group_li_ids = group.get("line_item_ids", [])
